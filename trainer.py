...
#        self.writer, self.log_txt = logger(self.exp_path, self.exp_name, self.pwd, 'exp', resume=cfg.RESUME)
    ...
    def forward(self):
        ...
        for epoch in range(self.epoch,cfg.MAX_EPOCH):
            ...
            # validation
            if (epoch%cfg.VAL_FREQ==0 and epoch > 0) or epoch>cfg.VAL_DENSE_START:
                self.timer['val time'].tic()
                if self.data_mode in ['SHHA', 'SHHB', 'QNRF', 'UCF50','Mall','FDST']:
            ...
            torch.save(self.net.cpu().state_dict(),"./weights/Pre_model_{}.pth".format(epoch+1))
            self.net.cuda()
